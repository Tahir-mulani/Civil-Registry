...
public partial class Customer_PancardReport : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        lblanapp.Visible = false;
        lblstatus.Visible = false;
        string name = Session["user"].ToString();
       // lblstatus.Text = name;
        cn.Close();
        cn.Open();
        cmd.CommandText = "Select status from PanCard where name = '" + name + "'";
        cmd.Connection = cn;
...
protected void  btngenerate_Click(object sender, EventArgs e)
{
    ...
    string type ="Pan Card";
    cn.Open();
    SqlDataAdapter da= new SqlDataAdapter("select * from PanCardoutput where type = '"+type+"' and name='"+name+"'",cn);
    da.Fill(ds,"PanCardoutput");
    da.Fill(ds);
    crpt.Load(Server.MapPath(@"~\Report\Pancardoutput.rpt"));
